...
function InboxView() {
  ...
  return (
    <div className="flex h-[calc(100vh-80px)] bg-secondary/30 rounded-xl overflow-hidden">
      {/* Email List */}
      <div className={`${selectedEmail ? 'hidden md:flex' : 'flex'} flex-col w-full md:w-[400px] lg:w-[450px] border-r border-border/50 bg-white/50`}>
        {/* Header */}
        <div className="p-4 border-b border-border/50">
          ...
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2 bg-secondary/50 border border-border/50 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-accent/30"
            />
          </div>
              ...
              {emails.map((email) => (
                <motion.div
                  ...
                  className={`flex items-start gap-3 p-4 border-b border-border/30 cursor-pointer transition-colors ${
                    email.is_read ? 'bg-transparent hover:bg-secondary/30' : 'bg-accent/5 hover:bg-accent/10'
                  } ${selectedEmail?.id === email.id ? 'bg-accent/10' : ''}`}
                >
                  {/* Read indicator */}
                  <button
                    ...
                    <div className="mb-6 p-4 bg-secondary/30 rounded-lg">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {selectedEmail.attachments.map((att, idx) => (
                          <div
                            key={idx}
                            className="flex items-center gap-2 px-3 py-1.5 bg-white rounded border border-border/50 text-sm"
                          >
                            <IconPaperclip className="w-3 h-3 text-muted" />
