...
const NAV_GROUPS: { label: string; items: { id: NavSection; label: string; icon: React.ReactNode; badge?: string; badgeWarn?: boolean }[] }[] = [
  ...
  { label: 'Préférences', items: [{ id: 'notifications', label: 'Notifications', icon: <IconBell className="w-3.5 h-3.5" /> }, { id: 'heures', label: 'Heures de travail', icon: <IconCalendar className="w-3.5 h-3.5" /> }] },
];
      ...
      <div className="min-h-[calc(100vh-8rem)]">
        ...
        >
          ...
          <div className="mt-auto pt-4 border-t border-default">
            <div className="flex items-center gap-2 px-3 py-2.5 bg-success/10 border border-success/20 rounded-lg !text-success !text-sm font-medium">
        ...
        <div className="lg:hidden flex gap-1 p-4 overflow-x-auto bg-card border-b border-default">
          {NAV_GROUPS.flatMap((g) => g.items).map((item) => (
            <button
              key={item.id}
              onClick={() => setActiveSection(item.id)}
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg whitespace-nowrap !text-xs font-medium transition-all ${
